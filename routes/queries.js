...
function createUser(req, res, next) {
  ...
  db
    ...
    .then(function(data) {
      res
        .status(200)
        .json({ status: "success", message: "New user created", id: data.id });
    })
    .catch(function(err) {
  ...
  db
    .one(
      "insert into favorites(movie_id, user_id, title, poster_path, release_date, vote_average, overview)" +
        "values(${movie_id}, ${user_id}, ${title}, ${poster_path}, ${release_date}, ${vote_average}, ${overview}) returning id",
      req.body
    )
    .then(function(data) {
      res
...
function deleteFavorite(req, res, next) {
  ...
  var user_id = parseInt(req.params.id);
  db
    .result("delete from favorites where user_id = $1 and movie_id = $2", [
      user_id,
      movie_id
    ])
